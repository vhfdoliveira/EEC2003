 ...
 * mpirun --nodes=2 --time=0-0:5 histogram_MPI_OpenMP <image>
 */
...
int main(int argc, char** argv) {
	...
	#pragma omp parallel 
	{
		...
		//TODO: check when this division isn't integer
		int local_cols = image.width() / nthreads;
		
		cout << "Thread " << thread_id << " of " << nthreads << " from process " << rank  
			 <<	" says: Hello World!" << endl;
		
		//calculate histogram of each "sub-image"
		for(int x = thread_id*local_cols; x < (thread_id+1)*local_cols; x++){
		...
		cout << endl << "Total of local pixels: " << local_cols * local_rows << endl;
	}
