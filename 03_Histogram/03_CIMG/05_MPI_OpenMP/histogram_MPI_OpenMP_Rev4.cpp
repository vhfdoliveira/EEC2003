 ...
 * 
 * Compile with:
 * mpic++ -o histogram_MPI_OpenMP_Rev4 histogram_MPI_OpenMP_Rev4.cpp -Wall -Dcimg_display=0 -Dcimg_use_jpeg -ljpeg -fopenmp
 * 
 * Use:
 ...
 * 
 * 	Rev4:
 * 		- The code is basically the same as the one of Rev1, but the openmp parallelization defined outside the outer loop, which means that the openmp will define how to handle the parallelization of that portion of code.
 * 		- Note the use of the collapse(2) function, which will transform the two nested loops in one bigger loop
 * 		- Note that each process will remain with the division of rows
 * 
 * 	Rev3:
...
int main(int argc, char** argv) {
	...
	if( rank == 0 ){
		...
		if( output ){
			...
			string csv_name = string("outputs/Rev4/") + string("output");
