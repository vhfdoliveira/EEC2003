 ...
 * 
 * Rev3:
 * 	- Included the collapse(2) in the pragma for (http://pages.tacc.utexas.edu/~eijkhout/pcse/html/omp-loop.html)
 * 	- Once the collapse(2) transforms the 2 nested loops in one big loop, when were specified a matrix size over 2^32 the program showed wrong results, because the size of the loop was bigger then a regular int.
 * 	- To fix this problem, the size of the iteration variables was declared as long int.
 * 
 * Rev2:
